...
static void setdisplaystart(int address)
{
    ...
    __svgalib_delay();
    port_out_r(ATT_IW, (address & 7));
    /* write sa0-2 to bits 0-2 */
    address >>= 3;
    port_outw_r(__svgalib_CRT_I, 0x0d + (address & 0x00ff) * 256);	/* sa0-sa7 */
    port_outw_r(__svgalib_CRT_I, 0x0c + (address & 0xff00));	/* sa8-sa15 */
}

static void setlogicalwidth(int width)
{
    port_outw_r(__svgalib_CRT_I, 0x13 + (width >> 3) * 256);	/* lw3-lw11 */
}
