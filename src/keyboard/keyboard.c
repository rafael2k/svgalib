...
static char *kbd_process_option(int option, int mode, char **nptr) {
  char *ptr;
  short type   = 0;
  short ignore_type   = 0;
...
#ifdef DEBUG_KEYBOARD
  fprintf(stderr," Fake Mouse Event: scancode: %d;  type: %d;  data: %d;  flags: %d\n", scancode, (int)type, data, (int)flags);
#endif
	   if ( ! expand_events(n))
	     return ptr;
...
int keyboard_init_return_fd(void) {
    ...
    DTP((stderr,"keyboard_init_return_fd\n"));
    ...
    if(__svgalib_novccontrol) {
        __svgalib_kbd_fd = open("/tmp/svga_kbd",O_RDONLY);
        freopen("/tmp/svga_tty","r",stdin);
    } else {
...
/* present. I don't understand fcntl. */
  ...
  again:
...
#ifdef DEBUG_KEYBOARD
                    fprintf(stderr," event type: %d;  flags: %d;  data: %d; is_pressed: %d\n", (int)event->type, flags, event->data, (int)is_pressed);
#endif
...
#ifdef DEBUG_KEYBOARD
            fprintf(stderr,"\tfake_mouse_event triggered; but: %d; dx: %d; dy: %d; dz: %d\n",
                   but, dx, dy, dz);
#endif
          }
            }
	...
	if (((alt_state && functionkey_state)||(win_state && functionkey_state)) &&
			!__svgalib_novccontrol && !(translatemode & DONT_CATCH_ALT_Fx)){
	    /* VT switch. */
	    /* *** what about F11 & F12? */
	    int j, vt = 0;
	    for (j = 0; j < 12; j++)
...
static char *kbd_load_keymap(char *ptr)
{
    ...
    if((keymapfile = fopen(ptr, "rt"))) {
        ...
        /* Initialize keymap */
        for (i = 0; i < NR_KEYS; keymap[i] = i, i++);
