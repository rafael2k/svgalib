...
static int  m_old_modem_info;		/*  original state of DTR/RTS */
...
static char* mouse_process_option(int option, int mode, char **nptr) {

 char *ptr;
 int newmtype;

#ifdef DEBUG_ACCEL
  fprintf(stderr,"Processing option %d (%s)\n", option, mouse_config_options[option]);
#endif
...
/*------------------------------------------------------------------*/
    ...
    if (m_maxdelta < 0  &&  !m_force) {
      fprintf(stderr, "svgalib: mouse_maxdelta value '%d' is invalid, should be > 0\n", m_maxdelta);
      m_maxdelta = DEFAULT_MAXDELTA;
    }

    if (m_accel_maxdelta < 0  &&  !m_force) {
      fprintf(stderr, "svgalib: mouse_accel_maxdelta value '%d' is invalid, should be > 0\n", m_accel_maxdelta);
	...
	m_accel_offset = DEFAULT_ACCEL_OFFSET;
      ...
      if (!m_force) {
         if (m_accel_thresh <= 0  ||  m_accel_thresh > 200) {
            fprintf(stderr, "svgalib: mouse_accel_thresh value '%d' is invalid, should be between 0 and 200\n", m_accel_thresh);
...
#ifdef DEBUG_ACCEL
	    fprintf(stderr,"  %d => %f (%f)\n", i, (double)m_accel_powertable[i] * i,
...
#ifdef DEBUG_ACCEL
    fprintf(stderr,"m_accel_type: %d\n",	(int)m_accel_type);
    fprintf(stderr,"m_force: %d\n",		m_force);
    fprintf(stderr,"m_accel_thresh: %d\n",	m_accel_thresh);
    fprintf(stderr,"m_accel_offset: %d\n",	(int)m_accel_offset);
    fprintf(stderr,"m_accel_mult: %f\n",	(double)m_accel_mult);
    fprintf(stderr,"m_accel_power: %f\n",	(double)m_accel_power);
    fprintf(stderr,"m_maxdelta: %d\n",		m_maxdelta);
    fprintf(stderr,"m_accel_maxdelta: %d\n",	m_accel_maxdelta);
#endif
...
*/
    ...
    if (m_type == MOUSE_BUSMOUSE || m_type == MOUSE_PS2 
    		|| m_type == MOUSE_IMPS2 || m_type == MOUSE_GPM
		|| m_type == MOUSE_DRMOUSE4DS || m_type == MOUSE_EXPPS2)
	m_modem_ctl = 0;
	...
	ms_setspeed(1200, m_baud, cflag[m_type]);
    } else if (m_type == MOUSE_WACOM_GRAPHIRE) {
    	m_baud = 9600;
    	ms_setspeed(1200, m_baud, cflag[m_type]);
    	/* Reset baud rate */
    	write(__svgalib_mouse_fd, "\r$", 2);
    	usleep(250000);
    	/* Reset tablet */
    	write(__svgalib_mouse_fd, "\r#", 2);
    	usleep(75000);
    	/* Set hardware filtering */
    	write(__svgalib_mouse_fd, "\rSU3", 4);
    	usleep(75000);
    	/* Start sending coordinates */
    	write(__svgalib_mouse_fd, "\rST\r", 4);
    } else if (m_type == MOUSE_IMPS2 || m_type == MOUSE_DRMOUSE4DS) {
	/* Initialize the mouse into wheel mode */
	write(__svgalib_mouse_fd, "\363\310\363\144\363\120", 6);
    } else if (m_type == MOUSE_EXPPS2) {
	write(__svgalib_mouse_fd, "\363\310\363\310\363\120", 6);
	...
	mouse_caps.buttons = MOUSE_FOURTHBUTTON;
...
/*  int but; */
  ...
  handle_packets:
	...
	((char) (buf[i] & ~0x33) == 0)) { /* s/23/33/, for 4-but trackman */
    ...
    if ((m_type != MOUSE_PS2)&&(m_type != MOUSE_IMPS2)
	&&(m_type != MOUSE_EXPPS2)&&(m_type != MOUSE_SPACEBALL)
	...
	dy = (char) (((buf[i] & 0x0C) << 4) | (buf[i + 2] & 0x3F));
        if((dx==0)&&(dy==0)&&(but==(prev&~MOUSE_MIDDLEBUTTON)))
            but=prev^MOUSE_MIDDLEBUTTON; else
	...
	if (buf[i] & 0x20 && dz) /* stylus has pressure */
		...
		if (buf[i+6] & 0x40)
            ...
            /* RX-axis */
            if(m_wheel_delta) {
                drx = ((wheel < 0) ? (-m_wheel_delta) : m_wheel_delta);
#ifdef DEBUG_WHEEL
            ...
            if(m_fake_kbd_events && __svgalib_keyboard_eventhandler) {
                /* Fake keypresses... */
                int key = ((wheel < 0) ? m_fake_upscancode : m_fake_downscancode);
#ifdef DEBUG_WHEEL
