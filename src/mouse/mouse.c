...
extern char *__svgalib_mouse_device;
extern int __svgalib_mouse_flag;
...
int mouse_init(char *dev, int type, int samplerate)
{
    if (mouse_open)
        return 0;
    if(dev==NULL) {
        dev = __svgalib_mouse_device;
        type = __svgalib_mouse_flag;
        samplerate=MOUSE_DEFAULTSAMPLERATE;
    }
